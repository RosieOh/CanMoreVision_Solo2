...
<%
    ...
    //3. 조회수 처리
    int count=0;
    String sql = "UPDATE applimentboard SET cnt=cnt+1 WHERE bno=?";
    ...
    sql = "select * from applimentboard where bno=?";
    pstmt = con.prepareStatement(sql);
    pstmt.setInt(1, bno);
    ...
    if(rs.next()){
        ...
        aboard.setContent(rs.getString("content"));
    ...
    sql = "SELECT cno, author, resdate, content FROM applimentComment WHERE bno=? ORDER BY cno DESC LIMIT 0, 10";
    pstmt = con.prepareStatement(sql);
    pstmt.setInt(1, bno);
    rs = pstmt.executeQuery();
    ...
    <div class="contents" id="contents">
        <div class="content_header">
            <div class="breadcrumb">
                <p><a href="<%=path5 %>">Home</a> &gt; <a href="<%=path5 %>/board/applimentBoard/ApplimentBoardList.jsp"> 입시 자료집  </a> > <span> 입시 자료집 상세보기 </span> </p>
                <h2 class="page_tit"> 입시 자료집 상세보기 </h2>
            </div>
        </div>
        <section class="page" id="page1">
            <div class="page_wrap">
                ...
                <table class="view_detail color">
                    <tbody>
                    <% if(sid.equals("admin") || sid.equals(aboard.getAuthor())) { %>
                    <tr>
                        <td colspan="3">
                            <a href="/board/applimentBoard/delApplimentBoard.jsp?bno=<%=bno%>&target=applimentboard" class="btn"> 삭제 </a>
                            <a href="/board/applimentBoard/updateApplimentBoard.jsp?bno=<%=bno%>" class="btn"> 수정 </a>
                        </td>
                    </tr>
                    <% } %>
                    <tr>
                ...
                <form action="<%=path5 %>/comment/addApplimentCommentPro.jsp" method="post">
                ...
                <div class="comment_detail">
                    <div class="comment_list">
                        ...
                        <a href="<%=path5 %>/comment/delApplimentCommentPro.jsp?bno=<%=bno%>&cno=<%=cmt.getCno()%>" class="btn"> [ 삭제 ] </a>
                        <% } %>
