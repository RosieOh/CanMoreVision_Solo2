...
<%
    ...
    // 페이징 처리 - 전체 페이지 수 구하기
    String sql = "SELECT COUNT(*) as 'count' FROM careerboard";
    ...
    sql = "SELECT * FROM careerboard ORDER BY bno DESC";
    pstmt = conn.prepareStatement(sql);
    pstmt.setInt(1, 10*(pageNo-1));
    rs = pstmt.executeQuery();

    List<CareerBoard> careerlist = new ArrayList<>();
    while(rs.next()) {
        CareerBoard cBoard = new CareerBoard();
        cBoard.setBno(rs.getInt("bno"));
        cBoard.setTitle(rs.getString("title"));
        cBoard.setAuthor(rs.getString("author"));
        cBoard.setCnt(rs.getInt("cnt"));
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        Date d = sdf.parse(rs.getString("resdate"));
        cBoard.setResdate(sdf.format(d));
        careerlist.add(cBoard);
    }
    ...
    <div class="contents" id="contents">
        <div class="content_header">
            <div class="breadcrumb">
                <p><a href="<%=path5 %>">Home</a> &gt; <span> 중학생 진로 자료 </span> </p>
                <h2 class="page_tit"> 중학생 진로 자료  </h2>
            </div>
        </div>
        <section class="page" id="page1">
            <div class="page_wrap">
                <p class="content_tit"> 중학생 진로 자료 </p>
                <hr>
                <div class="board_list_wrap">
                    <div class="board_list">
                        ...
                        <% for(CareerBoard careerBoard: careerlist) { %>
                        <div>
                            <div class="bno"> <%=careerBoard.getBno()%> </div>
                            <% if(sid!=null) { %>
                            <div class="qTitle"> <a href="<%=path5%>/board/careerBoard/getCareerBoard.jsp?bno=<%=careerBoard.getBno() %>"><%=careerBoard.getTitle() %></a> </div>
                            <% } else { %>
                            <div class="qTitle"><%=careerBoard.getTitle() %></div>
                            <% } %>
                            <div style="width: 22%"> <%=careerBoard.getAuthor()%> </div>
                            <div class="cnt"> <%=careerBoard.getCnt()%> </div>
                            <div style="padding-left: 80px;"> <%=careerBoard.getResdate()%> </div>
                        </div>
                        <% } %>
                        <% if(count == 0) { %>
                        <div>
                    ...
                    <div class="board_page">
                        <a href="<%=path5%>/board/careerBoard/CareerBoardList.jsp?page=1" class="bt first"> &lt;&lt; </a>
                        <a href="<%=path5%>/board/careerBoard/CareerBoardList.jsp?page=<%=pageNo-1 < 1 ? 1 : pageNo-1%>" class="bt prev"> &lt; </a>
                        <%  for(int p : pageList) {  %>
                        <a href="<%=path5%>/board/careerBoard/CareerBoardList.jsp?page=<%=p%>" class="num <%=(p==pageNo) ? "on" : ""%>"> <%=p%> </a>
                        <%  } %>
                        <a href="<%=path5%>/board/careerBoard/CareerBoardList.jsp?page=<%=pageNo+1 > totalPage ? totalPage : pageNo+1%>" class="bt next"> &gt; </a>
                        <a href="<%=path5%>/board/careerBoard/CareerBoardList.jsp?page=<%=totalPage%>" class="bt last"> &gt;&gt; </a>
                    </div>
                    <div class="btn_group" style="margin:0; width:1000px; padding-top:25px; ">
                        <%-- 공지사항이므로 관리자만 글 추가 기능(링크)이 적용되도록 설정 --%>
                        <% if(sid!=null) { %>
                        <a href="/board/careerBoard/addCareerBoard.jsp" class="inbtn" style="float:right; ">글쓰기</a>
                        <% } else { %>
