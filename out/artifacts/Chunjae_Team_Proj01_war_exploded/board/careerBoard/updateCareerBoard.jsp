...
<%
    ...
    String sql = "select * from careerboard where bno=?";
    ...
    if(rs.next()){
        careerBoard.setBno(rs.getInt("bno"));
        careerBoard.setTitle(rs.getString("title"));
        careerBoard.setContent(rs.getString("content"));
        careerBoard.setAuthor(rs.getString("author"));
        careerBoard.setResdate(rs.getString("resdate"));
        careerBoard.setCnt(rs.getInt("cnt"));
    }
    ...
    <div class="contents" id="contents">
        <div class="content_header">
            <div class="breadcrumb">
                <p><a href="<%=path5 %>">Home</a> &gt; <span> 중학생 진로 게시판 </span> </p>
                <h2 class="page_tit"> 중학생 진로 게시판 </h2>
            </div>
        </div>
        <section class="page" id="page1">
            <div class="page_wrap">
                <h2 class="page_tit">중학생 진로 게시판 수정하기</h2>
                <hr>
                <!-- 5. Board 객체의 내용을 폼의 각 컨트롤(input/textarea)에 바인딩하여 출력 -->
                <form action="updateCareerBoardPro.jsp" method="post">
                    <table class="tb1">
                        <tbody>
                        <tr>
                            <th>글 번호</th>
                            <td><input type="text" name="bno" id="bno" class="indata" value="<%=careerBoard.getBno() %>" readonly></td>
                        </tr>
                        <tr>
                            <th>글 제목</th>
                            <td><input type="text" name="title" id="title" class="indata" value="<%=careerBoard.getTitle() %>" required></td>
                        </tr>
                        <tr>
                            <th>글 내용</th>
                            <td><textarea rows="10" cols="80" name="content" id="content" class="indata2"><%=careerBoard.getContent() %></textarea></td>
                        </tr>
                        <tr>
                            <td colspan="2">
                                <%-- 6. 수정하기(submit)을 누르면, 수정처리로 넘기기 --%>
                                <a href="javascript:history.go(-1)" class="inbtn"> 이전 페이지 </a>
                                <% if(sid.equals("admin") || sid.equals(careerBoard.getAuthor())) { %>
                                    <input type="submit" value="글수정" class="inbtn">
                                <a href="/board/applimentBoard/delApplimentBoard.jsp?bno=<%=careerBoard.getBno() %>" class="inbtn">글 삭제</a>
                                <% } %>
                            </td>
                        </tr>
                        </tbody>
